...
`create_transactions` 只接受 `mcp:write` PAT，并要求非空的 `idempotency_key`。调用方应传入目标 `ledger_id`，普通交易使用 `account_id`，转账同时使用 `from_account_id` 和 `to_account_id`。服务端最多 200 笔一批、每 50 笔使用稳定的幂等分块键；同一批次安全重试不会重复创建已经成功的分块。幂等键应绑定调用方自己的不可变审核计划哈希，而不是临时随机值。
