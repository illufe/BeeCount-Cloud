...
MCP 是 Anthropic 推出的 LLM-工具集成协议。BeeCount Cloud 内置一个 MCP server,把账本能力暴露成 21 个 tool:
...
- **3 个 account tool**:`create_account` / `update_account` / `delete_account`(需二次确认)
...
3. 点 **新建 Token**:
   ...
   - **授权范围**:
     ...
     - `mcp:account_write` — LLM 只能维护账户,不能写交易；适合长期账户维护
...
| Scope 分离 | `mcp:read` / `mcp:write` / `mcp:account_write` 独立勾选;只 read 或 account write 不会被升权成交易 write |
| 危险操作 | `delete_transaction` 必须传 `confirm=true`,首次调用返"待确认"占位符,LLM 必须跟用户确认后再调一次 |
| 账户删除 | `delete_account` 必须显式传 `ledger_id`、`account_id` 和 `confirm=true`;存在关联交易时服务端拒绝 |
...
| `create_transactions` | **批量**新建交易(导入正解,一次提交多笔) | transactions(list), ledger_id, idempotency_key |
...
### Account tools(需要 `mcp:account_write`)
...
`create_transactions` 只接受 `mcp:write` PAT，并要求非空的 `idempotency_key` 和每笔交易的显式 `happened_at`。调用方应传入目标 `ledger_id`，普通交易使用 `account_id`，转账同时使用 `from_account_id` 和 `to_account_id`；名称和 ID 同时传入时必须指向同一账户，ID-only 请求会补齐账户名称。跨币种转账目前直接拒绝。服务端最多 200 笔一批、每 50 笔使用绑定整批长度的稳定幂等分块键；同一批次安全重试不会重复创建已经成功的分块，并会返回已重放分块的交易 ID。幂等键应绑定调用方自己的不可变审核计划哈希，而不是临时随机值。
...
**问:LLM 调账户 tool 报 "PAT missing required scope: mcp:account_write"**

- 创建或编辑 PAT 时单独勾选 `mcp:account_write`。它只允许账户维护,不会授予交易写权限。
